...
function rangesForDate(date, schedule) {
...
function requestStaffMismatch(requestedStaffId, bookingStaffId) {
    return Boolean(requestedStaffId && bookingStaffId && requestedStaffId !== bookingStaffId);
}
export const defaultAvailabilityProvider = ({ date, service, staffId, schedule, bookings, configuration }) => {
    const now = Date.now();
    const earliest = now + configuration.minNoticeMinutes * 60000;
    const latest = now + configuration.bookingWindowDays * 86400000;
        ...
        for (let minute = rangeStart; minute + duration <= rangeEnd; minute += interval) {
            ...
            const overlappingBookings = bookings.filter((booking) => {
                if (booking.status === "cancelled")
                    return false;
                if (requestStaffMismatch(staffId, booking.staffId))
                    return false;
...
export function bookingConflicts(candidate, bookings) {
    ...
    const overlappingBookings = bookings.filter((booking) => {
        if (booking.status === "cancelled")
            return false;
        if (requestStaffMismatch(candidate.staffId, booking.staffId))
            return false;
        const bookingStart = new Date(String(booking.metadata?.blockedStartsAt ?? booking.startsAt)).getTime();
        const bookingEnd = new Date(String(booking.metadata?.blockedEndsAt ?? booking.endsAt)).getTime();
        return overlaps(start, end, bookingStart, bookingEnd);
    });
    if (overlappingBookings.some((booking) => booking.serviceId !== candidate.serviceId))
        return true;
    return overlappingBookings.filter((booking) => booking.serviceId === candidate.serviceId).length >= capacity;
}
export function bookingCountByDate(bookings, timeZone) {
