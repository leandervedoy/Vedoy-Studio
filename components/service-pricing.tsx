...
const subscriptions = [
  { name: "IT-hjelp", price: "150 kr/mnd", description: "En enkel inngang til Vedøy Growth og trygg hjelp når noe stopper opp.", features: ["Vedøy Growth og aktive Growth-apper inkludert", "2 timer IT-hjelp inkludert per måned", "Oppmøte etter behov", "Telefon eller Teams når mulig", "Ekstra 30 min: 100 kr"] },
  { name: "IT + drift", price: "450 kr/mnd", description: "For bedrifter som vil ha mer hjelp og en løsning som holdes i orden.", features: ["Vedøy Growth og aktive Growth-apper inkludert", "6 timer IT-hjelp inkludert", "Nettsidehosting og drift", "2 innholdsoppdateringer per måned", "Ekstra 30 min: 80 kr", "Feil på Vedøy-laget løsning rettes uten ekstra kostnad"] , featured: true},
  { name: "Vedøy Growth", price: "750 kr/mnd", description: "En samlet bedriftsplattform for drift, kunder, oppgaver og videre vekst.", features: ["Hele Vedøy Growth-pakken inkludert", "6 timer IT-hjelp inkludert", "Nettsidehosting og drift", "3 oppdateringer per måned", "Ekstra time: 50 kr", "Fordeler på nye Vedøy-prosjekter"] },
  { name: "Growth Pro", price: "Fra 1 250 kr/mnd", description: "For team som trenger mer kapasitet, flere brukere og tettere oppfølging.", features: ["Hele Vedøy Growth-pakken inkludert", "10 timer IT-hjelp inkludert", "Flere ansatte og roller", "Prioritert support", "Mer støtte og videreutvikling", "Egen plan etter møte"] }
];
...
export function ServicePricing({ type }: { type: PricingType }) {
  ...
  return <section className="website-pricing" id="priser">
    ...
    <section className="business-subscriptions"><div className="business-subscriptions__heading"><div><p className="editorial-kicker lime">BEDRIFTSABONNEMENTER</p><h2>Én avtale.<br /><em>Mer ro.</em></h2></div><p>Alle bedriftsabonnement inkluderer Vedøy Growth og de ferdige Growth-applikasjonene som er aktive i avtalen. Dette gjelder på tvers av nettsider, webapper, nettbutikker, profilprodukter og andre Vedøy-tjenester.</p></div><div className="business-subscriptions__grid">{subscriptions.map((plan) => <article className={plan.featured ? "is-featured" : undefined} key={plan.name}><small>{plan.featured ? "ANBEFALT" : "VEDØY BEDRIFT"}</small><h3>{plan.name}</h3><strong>{plan.price}</strong><p>{plan.description}</p><ul>{plan.features.map((feature) => <li key={feature}>{feature}</li>)}</ul></article>)}</div><div className="business-subscriptions__upgrade"><strong>Fast prosjektpris kommer i tillegg</strong><p>Nettsider, apper, nettbutikker, design og andre nye leveranser bestilles som en separat fast prosjektpris etter avklart omfang. Høyere abonnement gir mer støtte, kapasitet og oppfølging – uten at vi gjør avtalen unødvendig komplisert.</p><p><strong>Omsetning over 500 000 kr per år?</strong> Da må virksomheten velge en høyere Growth-plan. Vi avklarer riktig nivå før avtalen eller neste fornyelse, basert på størrelse, behov og supportmengde.</p><Link href="/#kontakt" className="editorial-button">Be om gjennomgang <span>↗</span></Link></div></section>
  </section>;
}
