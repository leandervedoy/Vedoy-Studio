...
export function HostingConfigurator({ initialDomain = "", initialDomainMode = "existing", stripeMode, domainProviderReady }: Props) {
  ...
  async function checkout() {
    ...
    if (!contact.name.trim() || !contact.email.trim()) return setError("Skriv inn navn og e-post før du sender forespørselen.");
    ...
    try {
      const response = await fetch("/api/checkout/hosting", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ ...selection, ...contact }) });
      const data = await response.json() as { requestId?: string; error?: string };
      if (!response.ok || !data.requestId) throw new Error(data.error || "Bestillingen kunne ikke sendes.");
      window.location.assign(`/bestilling/fullfort?request_id=${encodeURIComponent(data.requestId)}`);
    } catch (reason) {
      setError(reason instanceof Error ? reason.message : "Bestillingen kunne ikke sendes.");
    ...
    <section className="hosting-configurator" id="konfigurer-hosting">
      <div className="hosting-configurator__heading">
        ...
        <h2>Bygg riktig<br /><em>kapasitet.</em></h2>
          <p>Prisene beregnes på nytt på serveren. Send inn en forespørsel, så avtaler vi oppstart og fakturering manuelt. Prisene er eks. mva.</p>
      </div>
      ...
      <div className="hosting-configurator__panel">
        ...
        <aside className="config-summary">
          <div className="config-contact-fields">
            <p className="editorial-kicker">KONTAKTINFORMASJON</p>
            <input value={contact.name} onChange={(event) => setContact((current) => ({ ...current, name: event.target.value }))} placeholder="Navn *" aria-label="Navn" required />
            <input type="email" value={contact.email} onChange={(event) => setContact((current) => ({ ...current, email: event.target.value }))} placeholder="E-post *" aria-label="E-post" required />
            <input value={contact.company} onChange={(event) => setContact((current) => ({ ...current, company: event.target.value }))} placeholder="Bedriftsnavn" aria-label="Bedriftsnavn" />
            <input value={contact.phone} onChange={(event) => setContact((current) => ({ ...current, phone: event.target.value }))} placeholder="Telefon" aria-label="Telefon" />
          </div>
          <div><span>Månedlig estimat</span><strong>{nok.format(price.monthlyNok)}</strong><small>eks. mva. · faktureres månedlig</small></div>
          ...
          <p className="config-message">Dette er en forespørsel, ikke en automatisk godkjenning. Vi kontakter deg for å bekrefte kapasitet, oppstart og månedlig fakturering.</p>
          {error ? <p className="config-error" role="alert">{error}</p> : null}
          <button type="button" className="config-checkout" onClick={checkout} disabled={checkoutState === "loading"}>{checkoutState === "loading" ? "Sender forespørsel …" : "Send hostingforespørsel"}</button>
          <small>Ingen betaling gjennomføres nå.</small>
        </aside>
      </div>
    </section>
  );
