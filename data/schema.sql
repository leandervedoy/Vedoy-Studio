...
do $$ begin
  if exists (select 1 from pg_roles where rolname = 'anon') then
    revoke all on table contact_requests, clothing_requests, studio_notes from anon;
  end if;
  if exists (select 1 from pg_roles where rolname = 'authenticated') then
    revoke all on table contact_requests, clothing_requests, studio_notes from authenticated;
  end if;
end $$;
