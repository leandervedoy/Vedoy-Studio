...
import { RequestStatusControl } from "@/components/studio/request-status-control";
    ...
    <section className="request-admin-grid">
      <article className="studio-panel request-admin-panel">
        ...
        <div className="request-list">{contacts.length ? contacts.map((item) => <article key={item.id}>
          ...
          {item.message ? <p>{item.message}</p> : null}<div className="request-item-footer"><time>{formatDate(item.createdAt)}</time><RequestStatusControl id={item.id} initialStatus={item.status} /></div>
