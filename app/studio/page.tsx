...
import { getOverview, isUsingDatabase, listStudioNotes } from "@/lib/repository";
import { formatDateTime } from "@/lib/utils";

export const dynamic = "force-dynamic";

export default async function StudioDashboardPage() {
  const [overview, notes] = await Promise.all([getOverview(), listStudioNotes()]);
  return <>
    <header className="dashboard-heading"><div><p className="eyebrow">GOD KVELD, LEANDER</p><h1>Studiooversikt</h1><p>Her er det viktigste på tvers av virksomheten akkurat nå.</p></div><div className="dashboard-heading__actions"><span className={isUsingDatabase() ? "data-mode is-live" : "data-mode"}><i />{isUsingDatabase() ? "PostgreSQL tilkoblet" : "Demodata i minnet"}</span><Link className="button button--dark" href="/studio/projects">+ Nytt prosjekt</Link></div></header>
